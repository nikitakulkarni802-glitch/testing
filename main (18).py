...
def generate_one_page_report(df, from_date, to_date, user_name):
    ...
    # ========== HEADER ==========
    fig.text(0.5, 0.97, "DRISHTI", fontsize=22, fontweight='bold', ha='center', color='#01579b', fontfamily='sans-serif')
    ...
    if total_cases > 0:
        ...
        if top_dept != "N/A" and 'DEPARTMENT' in df.columns:
            dept_count = df['DEPARTMENT'].value_counts().iloc[0]
            pct = (dept_count / total_cases) * 100
            insights.append(f"• {top_dept} department accounts for {pct:.1f}% of all cases.")
